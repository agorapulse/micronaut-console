...
You need to either set `console.enabled` property to `true`, or you can set `console.until`
to any ISO date in the future such as `2020-11-12T10:00:00Z`. The second option will enable to console only for a specific period.
...
WARNING: If you are using reverse proxy (e.g. Nginx on AWS Beanstalk) then you need to https://sergiodelamo.com/blog/host-and-ip-resolution-micronaut-load-balancer-elastic-beanstalk.html[set up the hostname resolution] correctly.
